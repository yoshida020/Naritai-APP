      ...
      <div className="relative max-w-[1200px] mx-auto px-4 py-8 md:py-16">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-12 mb-12">
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-sm text-[var(--footer-muted)] leading-relaxed mb-6">
              あなたの「なりたい」を実現する
            </p>
            <div className="flex gap-4">
              <a href="https://x.com/naritai_career" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/10 hover:bg-[var(--footer-bg)] flex items-center justify-center transition-colors">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              </a>
              <a href="https://www.instagram.com/naritai_career/" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/10 hover:bg-[var(--footer-bg)] flex items-center justify-center transition-colors">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
              ...
              <a href="https://line.me/R/ti/p/@099yapgw?oat_content=url&ts=07191130" target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full bg-white/10 hover:bg-[var(--footer-bg)] flex items-center justify-center transition-colors">
            ...
            <ul className="space-y-3 text-sm">
              {group.links.map((link) => (
                <li key={link.href}>
                  <a
                    href={link.href}
                    className="text-[var(--footer-muted)] hover:text-[var(--footer-text)] transition-colors flex items-center gap-2 group"
                    onClick={(e) => handleLinkClick(e, link.href)}
                  >
                    <span className="w-1 h-1 rounded-full bg-[var(--footer-bg)] opacity-0 group-hover:opacity-100 transition-opacity group-hover:scale-150"></span>
                    {link.label}
                  </a>
                </li>
              ))}
