...
export default function PricingSection() {
  ...
  // const features = ["キャリア開発支援", "月1回の面談", "レポート提供"];
    ...
    >
      <div className="max-w-[1200px] w-full mx-auto text-center h-full flex flex-col justify-center py-8 md:h-auto md:py-0">
        ...
        <div className="mb-8 max-w-[700px] mx-auto">
          <div
            className={`flex flex-col items-center gap-4 mb-6 ${isVisible ? 'animate-slide-in-from-top' : ''}`}
          >
            ...
            <div className="flex items-baseline gap-2 relative inline-block pb-1">
              ...
              <span className="text-[36px] font-bold text-[#2C3E50]">
                円
              </span>
              <span
                className={`absolute bottom-0 left-0 h-[24px] bg-gradient-to-r from-yellow-400/80 to-yellow-400/40 rounded-sm -skew-y-1 z-0 pointer-events-none transition-all duration-500 ${isVisible ? 'w-full' : 'w-0'}`}
                style={{ transitionDelay: '0s' }}
              />
            </div>
          </div>
          <div
            className={`text-4xl font-bold text-[#2C3E50] tracking-wide text-center lg:text-[48px] xl:text-[48px] md:text-xl md:text-3xl ${isVisible ? 'animate-slide-in-from-top' : ''}`}
          ...
          >
            ...
            <span className="relative inline-block pb-1 lg:hidden">
              毎月5社初回無料
              <span
                className={`absolute bottom-0 left-0 h-[14px] bg-gradient-to-r from-yellow-400/80 to-yellow-400/40 rounded-sm -skew-y-1 z-0 pointer-events-none transition-all duration-500 ${isVisible ? 'w-full' : 'w-0'}`}
                style={{ transitionDelay: '0.3s' }}
              />
            </span>
