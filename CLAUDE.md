...
このリポジトリで作業するときは、以下を必ず守ること。

### 禁止

- **`.env` および `.env.*` を開かない。** 中身の確認が必要なときは、値を出さずにキー名だけを見る（`cut -d= -f1 .env`）。
- **`.env.local` に EmailJS と Google Analytics のキーが入る。** 同様に開かない。
- **本番デプロイを実行しない。** Vercel 連携のため、`main` への push がそのまま本番反映になる。`vercel` コマンドも同様。

### 指示があったときだけ行う

- **コミットと push。** 作業の区切りで自己判断してはいけない。ユーザーが「コミットして」と言ったときだけ行う。
- **ファイルの削除・上書き。** 実行前に対象の中身を確認し、消える内容を伝えてから行う。
- ステージされていない変更（他の人が意図的に残している可能性がある）を、ついでに巻き込んでコミットしない。

### 作業の進め方

- **permissions や hooks を書き換えても、そのセッションでは有効にならない。** 反映にはユーザーによる再起動が必要。書いた直後に動作確認して「効いている」と判断しないこと。
- ユーザーはエンジニア初心者。専門用語を使うときは短い説明を添え、一度に多くの選択肢を並べない。
- 頼まれていない作業を見つけたら、実行せずに報告だけする。

## セキュリティ必須ルール

判断に迷ったら `SECURITY_CHECKLIST.md` を見る。作業のたびに以下を守ること。

- **このリポジトリは GitHub 上で公開されている**（`yoshida020/Naritai-APP` は Public）。
  push した内容は全世界から読める。一度 push した秘密の値は、あとで消しても漏れたものとして扱い、必ず再発行する。
- **`NEXT_PUBLIC_` を付けた環境変数はブラウザから丸見えになる。** 秘密にすべき値には絶対に付けない。
  サーバー側でしか使わない値は `NEXT_PUBLIC_` を付けず、Server Component か Route Handler の中でだけ読む。
- **EmailJS のキー（`NEXT_PUBLIC_EMAILJS_*`）は公開前提の設計**で、露出そのものは問題ない。
  ただし第三者に送信枠を使われないよう、EmailJS 管理画面側でドメイン制限とレート制限が要る。
  フォーム周りを触ったら、この設定の有無をユーザーに確認する。
- **依存パッケージを足す・上げるときは、最新の安定版か・EOL でないかを確認してから**バージョンを書く。
  作業後に `npm audit` を回し、結果を報告する。
- **問い合わせフォームに個人情報が流れる。** 入力値をログやエラーメッセージにそのまま出さない。
